...
def build_camera_only_prompt(sample):
    ...
    return "\n".join(
        [
            ...
            "- 차량은 자동차, 트럭, 버스, 승합차를 모두 포함한다.",
            "- 안전 표지판은 주의/안내/공사/통제 표지판과 애매한 입간판을 포함한다.",
            "- 주차콘은 라바콘/콘 형태 장애물을 뜻한다.",
            "- 벽은 실제 벽, 담장, 건물 외벽 같은 고정 구조물일 때만 고른다.",
            "- 확실하지 않으면 primary_object_ko는 '기타 장애물'로 둔다.",
            "- driving_reason_ko는 카메라에서 보이는 통로, 장애물, 사람 움직임, 접근/이격 단서만으로 쓴다.",
            "- 반드시 JSON만 출력한다.",
            '- 형식: {"scene_domain_ko":"실내|실외|불명", "primary_object_ko":"", "dynamic":"static|dynamic|unknown", "scene_summary_ko":"", "driving_reason_ko":"", "confidence":0.0}',
        ]
    )
